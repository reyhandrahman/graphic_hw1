	...
	for (int cameraIndex = 0; cameraIndex < cameraNumber; cameraIndex++)
	{
		...
		for (int y = 0; y < imageHeight; ++y)
		{
			for (int x = 0; x < imageWidth; ++x)
			{
				...
				float t;
				Material material;
				Vec3f unitNormalVector;

				//check ray-sphere intersection or ray-triangle intersection
				if (scene.isIntersected(ray, t, material, unitNormalVector))
				{
					//compute shadow rays
					Vec3i color = scene.computeShadow(ray, t, unitNormalVector, material, scene.max_recursion_depth);
					image[i++] = color.x;
					image[i++] = color.y;
					image[i++] = color.z;
				}
